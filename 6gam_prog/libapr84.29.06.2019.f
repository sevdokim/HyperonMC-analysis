...
C 
     ...
     &              /0.010, 0.050,   0.005, 0.005, 0.005/               ! kondr
...
C
      if(ngam.eq.3.and.mfit.eq.1)then  
         ipi0gamma=ipi0gamma+1                     ! save current event
...
C     pi0eta
C
      if(ngam.eq.4.and.mfit.eq.2)then  ! save current event
         ietapi0=ietapi0+1
...
C
      IF ( PCHI.gt.0.15) THEN
	   ...
	   ENDIF 
      ENDIF
C                                      My main energy dependent EFM histos
      do Je=0,9                     !=======================================
         if(ETYM.gt.1.+0.5*Je) CALL HF1(IHM+30+Je,EFM ,1.)     
      enddo
C    
Comg-
...
C Kondratyuk probabilities based on Kfit analysis
 ...
 850  Kfit = 0             ! Start of the probability Kfit analysis
C                            ----------------------------------------
...
C3!- +      probhyp(3).lt. HypProbCriteria4g(3) .and. 
     +      probhyp(2).lt. HypProbCriteria4g(2)) then
               Kfit = 1; goto 752; endif			     ! not used ; mind the 752
        if (
     +      probhyp(2).gt.GoodProbCriteria4g(2) .and. 
C3!- +      probhyp(3).lt. HypProbCriteria4g(3) .and. 
     +      probhyp(1).lt. HypProbCriteria4g(1)) then
	        Kfit = 2; goto 752; endif			     ! not used ; mind the 752
      endif
...
!commented ; kondr++{
!       CALL HBOOK1(IH+50+j,'MASS FIT Prob gt 0.10, E cut',2000,0., 2.,0.)
!       CALL HBOOK1(IH+60+j,'ETOT FIT Prob gt 0.10, E cut', 300,0.,15.,0.)
!       CALL HBOOK1(IH+70+j,'Pt   FIT Prob gt 0.10, E cut',1000,0., 2.,0.)
CEvd+
!       CALL HBOOK1(-(IH+50+j),'Mix MASS FIT Prob gt 0.10, E cut',2000,0.,
!      +     2.,0.)
