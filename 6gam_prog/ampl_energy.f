...
Cevd+
C          if(iener2(i).gt.8000) reject=.true.     ! Etot event rejection 
          if(iener2(i).gt.8500) reject=.true. ! Etot event rejection
          nx=(i-1)/24+1
          ny=(i-1)-(nx-1)*24+1
...
Cevd+
C          if(ieners(i).gt.8000) reject=.true.     ! Etot event rejection 
	  if(ieners(i).gt.8500) reject=.true. ! Etot event rejection
          nx=(i-1)/8+1
          ny=(i-1)-(nx-1)*8+1
          call hf2(-75200,float(nx),float(ny),float(IENERS(I)))
C         call hf1(-75400,float(IENERS(I)),1.)
Cevd          
...
c-	  write(*,*) ETOT
   ...
   40   CONTINUE
C
	CALL HF1(20020,float(iamptot),1.)
        CALL HF1(20021,ETOT,1.)
C
        nctot = ncellb + ncells
        myenergy=etot
        call hf1(5032,etot,1.)
        IF (ETOT.LT.ETOTL) RETURN
        if (iamptot.lt.amptotl) return
C
C-      write(*,*) etot,ETOTL,(clb2(i),i=1,4),(IENERS(I),i=1,4)
        if (ETOT.gt.10000.) return 
Cevd+
       if(reject) return  ! event rejection
Cevd-
       OK  =.TRUE.
       Eout= etot
       CALL HF1(20022,ETOT,1.)
C   
C-     call hf2(-70060 ,float(nctot),etot,1.)
C       
        nafnaf_energy=nafnaf_energy-1
C
