...
C
      ...
      DATA NEUMS,QNMS / 0.939565413, 0.882783165/
...
C
     ...
     +    probhyp(nhip) =  prob(real(chiq),nf)    ! here I save probabilities for all hypothesa, 
                                                  ! will process them after the cycle ;kondr ; 
						  ! NHIP = mfit = khp = khip(ihp) = khip(kftf(khp)) 
						  ! -- Number of current hypothesis ( currently being 
						  ! processed in the 500th cycle ) ; may be outdated ;kondr
...
C-112 IF ( MFIT.NE.0.AND.CHIQ.GE.FPAR(46)) GO TO 498
  112 IF ( MFIT.NE.0.AND.CHIQ.GE.FPAR(46).AND.NHIP.NE.2) GO TO 498      ! here we just skip the rest of the 
                                                                        ! cycle for 1st hyp if chi^2 is higher 
									! then _previous_ ones ; fpar(46) is a container 
									! for the minimum chiq value for the current event
C      
      IF ( NHIP.EQ.2) THEN
        CHIR = CHIQ                                   ! no need to use this variable, it's so temporary...used 
	                                              ! in the line below...!! must be replaced with real(chiq) ; k
        PCHI = PROB(CHIR,NF)
...
!                 mfit = 2; goto 752; endif                             ! not used ; mind the 752
 ...
 751  ihhh = 100000*khp
      if (ngam.eq.3.and.mfit.eq.2)    ihhh = 1000000 + 100000*khp       ! for the sake of consistancy I will write my cut 
                                                                        ! for eta-gamma in the another histogramm. however 
									! my cut for pi0gg will go to SA number because he 
									! doesn't have it
      RTYM = ETYM
C      
      if (ngam.eq.3) then
...
C
      ...
      CALL HF1(IHH+30, MMS    ,1.)
C    
Comg-
      IF (NGAM.eq.3.and.ETYM.gt.3.0) THEN
           IF (Efm.gt.0.722.and.Efm.lt.0.842) then
               call hf1(IHH+41, RTYM, 1.)
	       call hf1(IHH+43, Efm , 1.)
	       call hf1(IHH+45,-RT  , 1.)
	       call hf1(IHH+47, MMS , 1.)
	       call hf2(IHH+49, MMS ,-RT, 1.)
C         
	   ELSE if (Efm.gt.0.662.and.Efm.lt.0.902) then  
	       call hf1(IHH+42, RTYM, 1.)
               call hf1(IHH+44, Efm , 1.)
               call hf1(IHH+46,-RT  , 1.)
               call hf1(IHH+48, MMS , 1.)
               call hf2(IHH+50, MMS ,-RT, 1.)	        
          ENDIF
Cf2-      
...
C         
      ...
      ENDIF 
CKs-      
      IF (NGAM.eq.4.and.ETYM.gt.2.5) THEN
         IF (Efm.gt.0.472.and.Efm.lt.0.522) then
             call hf1(IHH+51, RTYM, 1.)
	     call hf1(IHH+53, Efm , 1.)
	     call hf1(IHH+55,-RT  , 1.)
	     call hf1(IHH+57, MMS , 1.)
	     call hf2(IHH+59, MMS ,-RT, 1.)
C       
	 ELSE if (Efm.gt.0.447.and.Efm.lt.0.547) then  
	     call hf1(IHH+52, RTYM, 1.)
             call hf1(IHH+54, Efm , 1.)
             call hf1(IHH+56,-RT  , 1.)
             call hf1(IHH+58, MMS , 1.)
             call hf2(IHH+60, MMS ,-RT, 1.)	      
        ENDIF 
      ENDIF            
C      
C-    write(*,*) 'chiq=',NGAM,CHIQ,PCHI
                                                          ! another temporary variable
      IF ( PCHI.LT.0.15) GO TO 750
      CALL HF1(HypModulator+IHH+26, EFM, 1.)
      CALL HF1(HypModulator+IHH+27, RTYM,1.)
      CALL HF1(HypModulator+IHH+31, MMS ,1.)
      IF ( -T .LT.0.25) THEN
...
C
      DO 20 Ig=2,4 
      IHH = Ig*100
      CALL HBOOK1(IHH+1,'Hyperon Esum	       ',150, 0.,15.E3,  0.)
      CALL HBOOK1(IHH+2,'Kinfit  Einit         ',500, 0., 2.E4 , 0.)
      CALL HBOOK1(IHH+3,'Kinfit  Ecorrected    ',500, 0., 2.E4 , 0.)
      CALL HBOOK1(IHH+4,'Kinfit  Ecor. for T   ',500, 0., 2.E4 , 0.)
...
C
      CALL HBOOK1(IH+50,'MisMas CHIQ lt 11.5    ', 250,0.,5.,0.)
      CALL HBOOK1(IH+51,'MisMas CHIQ lt 6.25    ', 250,0.,5.,0.)
      call hbook1(IH+53,'Mas pi0g 1C-fit Etot.gt.3.0',200,0.,2.,0.)
      call hbook1(IH+54,'Mas 2pi0 2C-fit Etot.gt.3.0',200,0.,2.,0.)
Comg- 
      if(Ig.eq.3) then
C
