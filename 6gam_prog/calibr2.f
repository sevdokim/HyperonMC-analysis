...
C-    write(*,*) 'ESHW(1,I),ESHW(2,I)',EMIN2,ESHW(1,I),ESHW(2,I)
...
100   CONTINUE
      CALL HF1(20040,FLOAT(NGAM),1.)
c
C!!!- ONLY NGAM =  3  events !!!!!!!
C-----------------------------------
C-      if  (NGAM.ne.3) then 
C-                      result_ok=.false.
C-	 	      return
...
c
      IF (NGAM.EQ.2) THEN
         IGAM(1) = 1
         IGAM(2) = 2
         CALL EFMASS(2,IGAM,EFMA,Esum,OK)            ! ;kondr
         IF (.NOT.OK) GOTO 999
C 
C!-      write(*,*) 'NGAM=',NGAM,Esum,EFMA
C	 
         CALL ANGLE(1,2,COS2G,ANG2G)
         if (nt.eq.0) then
            CALL HF1(20130+NSTEP,EFMA,1.)
            CALL HF1(20140+NSTEP,ANG2G,1.)
            if(one(1).eq.1.or.one(2).eq.1) call hf1(20132,efma,1.)
            if(one(1).eq.2.or.one(2).eq.2) call hf1(20133,efma,1.)
            if(one(1).eq.3.or.one(2).eq.3) call hf1(20134,efma,1.)
         else
            numpi0 = 1
            effmass(1) = efma
            ggangle(1) = ang2g
...
c--------------zapolnenie matrici dlia popravki energii-----
         if(ifpi0)then
            call hf1(20112,esum,1.)
               if(esum.ge.1000.) then             !  epl54.dat and less
CSdv-          if(esum.ge.1500.) then 
               call matr_ent(esh,1)
               CALL HF1(20381,EFMA  ,1.)
               CALL HF1(20382,esh(1),1.)
               CALL HF1(20382,esh(2),1.)
               CALL HF1(20383,esh(1)+esh(2),1.)
            endif
         endif
c         if(((EFMA.gt.pi0mass-(EFMAX-EFMIN).and.EFMA.le.EFMIN).or.
c     &       (EFMA.lt.pi0mass+(EFMAX-EFMIN).and.EFMA.ge.EFMAX)).and.
c     &        esum.ge.1500.) then 
c           call hf1(20112,esum,1.)
c           CALL HF1(20181,EFMA,1.)
...
c
            ...
            if (nt.eq.0) then
              CALL HF1(20180+NSTEP,EFMA,1.)
            else
              npisel = 1
              selmass(1) = efma
              selang(1)  = ang2g
C-            if (nttype.eq.0) call hfnt(333)
            endif
      ENDIF
c
...
C
  999 CONTINUE      
C-999 if (nttype.eq.1) call hfnt(333)
