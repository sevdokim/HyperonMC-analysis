...
CBeam-data targetmass,beammomentum /939.57,7000.0/
CBeam-data targetmass,beammomentum /939.57,7350.0/
CBeam-data targetmass,beammomentum /939.57,7150.0/
      data targetmass,beammomentum /939.57,7200.0/
...
C      
      ...
      IF (UU.lt.0.) UU=-UU   
C
C!!!->IF (TT.LE.0.) THEN		       !  T positive cut
...
C
C      if((repulse_energy + esum).gt.8000.) return    ! Etot event cut for fun ;kondr
      if((repulse_energy + esum).gt.8500.) return ! Etot event cut for fun ;kondr
