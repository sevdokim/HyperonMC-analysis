...
c*****************************************************
      ...
      real proton_mass, neutron_mass, repulse_energy                    ! ;kondr                                            ! ;kondr
...
C
      if((repulse_energy + esum).gt.8000.) return    ! for fun ;kondr
C-    if((repulse_energy + esum).gt.8500.) return    ! for fun ;kondr
...
c-------tol'ko K+
     ...
     &                (P1g_mix(3,icurrent,1) + P2g_mix(3,icycl,1))**2
               do counter=1,10   !energy slices cycle
                  energy_slice=(counter-1)*500+1500
